...
class TodoServicesImplTest {
    ...
    @Test
    @DisplayName("findTodosByTitle() debe retornar lista de todos que coinciden con el título")
    void findTodosByTitle_shouldReturnMatchingTodos() {
        // Given
        TodoEntity todo1 = new TodoEntity();
        todo1.setId(1L);
        ...
        TodoEntity todo2 = new TodoEntity();
        todo2.setId(2L);
        todo2.setTitle("Another New Todo");

        List<TodoEntity> expectedTodos = List.of(todo1, todo2);
        given(todoRepository.findByTitleContainingIgnoreCase("New")).willReturn(expectedTodos);

        // When
        List<TodoEntity> result = todoServices.findTodosByTitle("New");

        // Then
        assertNotNull(result);
        assertEquals(2, result.size());
        assertTrue(result.get(0).getTitle().contains("New"));
        assertTrue(result.get(1).getTitle().contains("New"));
        verify(todoRepository, times(1)).findByTitleContainingIgnoreCase("New");
    }

    @Test
    @DisplayName("findTodosByTitle() debe retornar lista vacía cuando no hay coincidencias")
    void findTodosByTitle_shouldReturnEmptyListWhenNoMatches() {
        // Given
        given(todoRepository.findByTitleContainingIgnoreCase("NonExistent")).willReturn(List.of());

        // When
        List<TodoEntity> result = todoServices.findTodosByTitle("NonExistent");
    ...
    @Test
    @DisplayName("findTodosByTitle() debe buscar ignorando mayúsculas/minúsculas")
    void findTodosByTitle_shouldSearchCaseInsensitive() {
        // Given
        List<TodoEntity> expectedTodos = List.of(sampleTodo);
        given(todoRepository.findByTitleContainingIgnoreCase("sample")).willReturn(expectedTodos);

        // When
        List<TodoEntity> result = todoServices.findTodosByTitle("sample");

        // Then
        assertNotNull(result);
        assertEquals(1, result.size());
        verify(todoRepository, times(1)).findByTitleContainingIgnoreCase("sample");
    }
}
